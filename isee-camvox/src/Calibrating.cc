...
namespace Camvox
{
    ...
    //**************************************************************************************************************************************************/
    /********************************************Calibration**************************************************************************/
Calibrating::Calibrating(string _strSettingPath, string _RGB_path,string _Pcd_path, string _Projection_type, bool _isEnhanceImg, bool _isFillImg): mbResetRequested(false), mbFinishRequested(false), mbFinished(true), mbStopped(false), mbStopRequested(false), mbNotStop(false)
{
...
#ifdef debug
  color3d(extrinsic_params, 1, "debug/test.pcd");                     
...
Mat Calibrating::convertByIntensity(const Vector6d &calib_params)
{
  ...
  cv::imwrite("/home/zyw/livox_cam_calib/debug/intensity.jpg", filed_img);
  cv::waitKey();
...
Mat Calibrating::convertByDepth(const Vector6d &calib_params,const bool is_fillImg)
{
  ...
  cv::imwrite("/home/zyw/livox_cam_calib/debug/depth.jpg", image_project);
  cv::waitKey();
#endif
  return filed_img;
}
        ...
        while (1)
        {
            if (mbCalibrating)
            {
                ...
                // update parameter 拋棄異常值（超過上線，或者超過下線），根據均值可信範圍，選擇是否接受標定的結果並跟新進配置文件
                {
                    ...
                    std::string init_info = "/home/zyw/livox_cam_calib/debug/cost:" + std::to_string(init_distance);
                    ...
                    for (int iter_count = 0; iter_count < search_round; iter_count++)
                    {
                        for (int round_index = 0; round_index < 3; round_index++)
                        {
                                    ...
                                    {
                           {
                                ...
                                float distance = costFunction(test_angle);
                                std::string info = "/home/zyw/livox_cam_calib/debug/cost:" + std::to_string(distance);
                        ...
                        //Calibrating.showOrgPointClouds("optimize rpy");
                        //Calibrating.showClouds(Calibrating.rgb_use_cloud_xyzrgb_,Calibrating.lidar_use_cloud_xyzrgb_);
            ...
            else
            {
               // cout << "iteration" << endl;
                ...
                // Motion tracking 實時跟蹤機器人的運動，根據圖像的灰度直方圖變化判斷是否停止。 若是，觸發標定線程
                {
