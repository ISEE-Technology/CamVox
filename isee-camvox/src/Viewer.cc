...
namespace Camvox
{
    ...
    void Viewer::Run()
    {
        ...
        pangolin::CreateWindowAndBind("Camvox: Map Viewer", 1024, 768);                               //! this code need to be changed !
        ...
        cv::namedWindow("Camvox: Current Frame");                                                        //! this code need to be changed !
...
/*********************************************************************************/
    bool Viewer::judgemovement( cv::Mat &pre_frame,  cv::Mat &curr_frame)
    {
        ...
        {
        if(contourArea(contours[i])<1300) //轮廓小于1000不要，认为是噪声
            continue;
        else                              //否则认为有一个移动轮廓
            count++;
        }    
        // 移动轮廓的个数/所有有轮廓的个数，注意：所有轮廓包含移动轮廓和噪声轮廓
        //超过10个移动轮廓认为车在移动, 屏蔽掉车不动，但有移动物体的情况//8
        if(count>=7)                               
            return true;
        else
            return false;
